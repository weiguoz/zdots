    ...
    config = function()
        ...
        vim.lsp.config("gopls", {
            on_attach = function(_, bufnr)
                ...
                vim.lsp.inlay_hint.enable(true, { bufnr = bufnr }) -- 开启 inlay hints
            end,
            settings = {
                gopls = {
                    ...
                    staticcheck = true,
                    hints = {
                        assignVariableTypes = false,   -- a := "x" 旁边不显示 string
                        rangeVariableTypes = true,    -- for k, v := range ... 显示类型
                        parameterNames = true,        -- 函数调用参数名提示
